...
implements Iterable<Object> {
    ...
    JSONArray(Collection<?> collection, int n, JSONParserConfiguration jSONParserConfiguration) {
        if (n > jSONParserConfiguration.getMaxNestingDepth()) {
        ...
        if (collection == null) {
            ...
            this.addAll(collection, true, n, jSONParserConfiguration);
        }
    }
    ...
    public JSONArray put(int n, Map<?, ?> map) throws JSONException {
        this.put(n, new JSONObject(map, new JSONParserConfiguration()));
        return this;
    }

    public JSONArray put(int n, Map<?, ?> map, JSONParserConfiguration jSONParserConfiguration) throws JSONException {
        this.put(n, new JSONObject(map, jSONParserConfiguration));
        return this;
    }
    ...
    private void addAll(Collection<?> collection, boolean bl, int n, JSONParserConfiguration jSONParserConfiguration) {
        this.myArrayList.ensureCapacity(this.myArrayList.size() + collection.size());
        if (bl) {
            for (Object obj : collection) {
                this.put(JSONObject.wrap(obj, n + 1, jSONParserConfiguration));
            }
        } else {
            for (Object obj : collection) {
                this.put(obj);
    ...
    private void addAll(Object object, boolean bl) throws JSONException {
        this.addAll(object, bl, 0);
    }

    private void addAll(Object object, boolean bl, int n) {
        this.addAll(object, bl, n, new JSONParserConfiguration());
    }

    private void addAll(Object object, boolean bl, int n, JSONParserConfiguration jSONParserConfiguration) throws JSONException {
        if (object.getClass().isArray()) {
            int n2 = Array.getLength(object);
            this.myArrayList.ensureCapacity(this.myArrayList.size() + n2);
            if (bl) {
                for (int i = 0; i < n2; ++i) {
                    this.put(JSONObject.wrap(Array.get(object, i), n + 1, jSONParserConfiguration));
                }
            } else {
                for (int i = 0; i < n2; ++i) {
            ...
            this.addAll((Collection)object, bl, n);
