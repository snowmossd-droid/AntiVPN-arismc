...
public class XML {
    ...
    private static boolean parse(XMLTokener xMLTokener, JSONObject jSONObject, String string, XMLParserConfiguration xMLParserConfiguration, int n) throws JSONException {
        ...
        if (object == GT) {
            ...
            if (xMLParserConfiguration.getForceList().contains(string4)) {
                ...
                if (!xMLParserConfiguration.shouldTrimWhiteSpace()) {
                    XML.removeEmpty(jSONObject2, xMLParserConfiguration);
                }
                jSONObject.accumulate(string4, jSONObject2);
            }
            return false;
    ...
    private static void removeEmpty(JSONObject jSONObject, XMLParserConfiguration xMLParserConfiguration) {
        if (jSONObject.has(xMLParserConfiguration.getcDataTagName())) {
            Object object = jSONObject.get(xMLParserConfiguration.getcDataTagName());
            if (object instanceof String) {
                if (XML.isStringAllWhiteSpace(object.toString())) {
                    jSONObject.remove(xMLParserConfiguration.getcDataTagName());
                }
                ...
                for (int i = jSONArray.length() - 1; i >= 0; --i) {
                    ...
                    if (!(object2 instanceof String) || !XML.isStringAllWhiteSpace(string = (String)object2)) continue;
                    jSONArray.remove(i);
                }
    ...
    private static boolean isStringAllWhiteSpace(String string) {
    ...
    public static JSONObject toJSONObject(Reader reader, XMLParserConfiguration xMLParserConfiguration) throws JSONException {
        ...
        XMLTokener xMLTokener = new XMLTokener(reader, xMLParserConfiguration);
    ...
    private static String toString(Object object, String string, XMLParserConfiguration xMLParserConfiguration, int n, int n2) throws JSONException {
        ...
        if (object instanceof JSONObject) {
            ...
            for (String string3 : jSONObject.keySet()) {
                ...
                if ("".equals(object3)) {
                    if (xMLParserConfiguration.isCloseEmptyTag()) {
                        stringBuilder.append(XML.indent(n2));
                        stringBuilder.append('<');
                        stringBuilder.append(string3);
                        stringBuilder.append(">");
        ...
        String string5 = string2 = n > 0 ? "\n" : "";
        if (string == null) {
            return XML.indent(n2) + "\"" + string4 + "\"" + string2;
        }
        if (string4.length() == 0) {
            return XML.indent(n2) + "<" + string + "/>" + string2;
        }
        return XML.indent(n2) + "<" + string + ">" + string4 + "</" + string + ">" + string2;
    }
